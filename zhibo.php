...
<?php
		$sourl = "http://ivi.bupt.edu.cn";
		$info = file_get_contents($sourl);
		$vname = "#<p>(.*?)</p>#";
		$vimg = "#<a class=\"icon1\" href=\"/hls/(.*?).m3u8\" target=\"_blank\"#";
		...
		$link = $imgarr[1];
		$fname = $namearr[1];
?><strong class="single-strong">高清电视在线直播</strong><ul class="mvul"><?php
		foreach ($fname as $key => $video) {
?><a href="jx/jxz.php?url=http://ivi.bupt.edu.cn/hls/<?php echo $link[$key];?>.m3u8"><li><?php echo $zname[$key];?></li></a><?php
		}
?></ul>
